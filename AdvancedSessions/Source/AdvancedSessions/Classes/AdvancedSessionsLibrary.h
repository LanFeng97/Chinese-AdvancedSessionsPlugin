...
public:
		...
		// 将FBPUniqueNetID转换为FUniqueNetIdRepl（可复制/网络同步的唯一网络ID）
		UFUNCTION(BlueprintPure, meta = (DisplayName = "唯一网络ID转可复制格式 (ToUniqueNetIDRepl)", CompactNodeTitle = "->", BlueprintAutocast), Category = "Online|AdvancedSessions|PlayerInfo|PlayerID")
